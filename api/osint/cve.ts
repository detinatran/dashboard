...
export default async function handler(req: Request) {
  ...
  if (!cve) return Response.json({ error: 'Missing cve parameter' }, { status: 400 });
  ...
  if (!/^CVE-\d{4}-\d{4,}$/i.test(cve)) {
    return Response.json({ error: 'Invalid CVE format. Expected: CVE-YYYY-NNNNN' }, { status: 400 });
  }
  ...
  try {
    ...
    return Response.json({ error: 'CVE lookup failed' }, { status: 500 });
  }
}
