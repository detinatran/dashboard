...
export default async function handler(req: Request) {
  ...
  if (!query) return Response.json({ error: 'Missing query parameter (IP, ASN number, or prefix)' }, { status: 400 });
  ...
  try {
    ...
    if (isIP) {
      ...
      return Response.json({ error: 'Unrecognized query format. Use IP address or AS number.' }, { status: 400 });
    }

    return Response.json(results);
  } catch {
    return Response.json({ error: 'BGP lookup failed' }, { status: 500 });
  }
}
