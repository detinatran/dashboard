...
function ipv4ToInt(ip: string): number {
  ...
  // Callers reach here only after parseIPv4 has validated four octets, but read
  // defensively so a malformed string scores 0 rather than NaN — NaN compares
  // false against every blocked range, which would defeat the guard.
...
function ipv4InBlocked(ip: string): boolean {
  ...
  for (const [net, bits] of IPV4_BLOCKS_TEXT) {
    ...
    const blockSize = bits === 0 ? 0x100000000 : 2 ** (32 - bits);
