...
function generateResidentialIP(): string {
  const pool = IP_POOLS[randomInt(IP_POOLS.length - 1)] ?? IP_POOLS[0]!;
  const [base0 = 0, base1 = 0] = pool.base;
  const [range0 = 0, range1 = 0] = pool.range;
  const octet3 = base1 + randomInt(range0);
  const octet4 = randomInt(range1);
  return `${base0}.${octet3}.${octet4 || 1}.${randomInt(254) + 1}`;
}

function randomUA(): string {
  return USER_AGENTS[randomInt(USER_AGENTS.length - 1)] ?? USER_AGENTS[0]!;
}
