...
// `radius` is accepted for call-site compatibility with the OSIRIS original but
// is not consulted: the region boxes below are coarse enough that a viewport
// radius never changes which of them a point falls into.
...
/** Region keys this module can serve. */
export function listCctvRegions(): string[] {
  return Object.keys(REGION_FETCHERS);
}

/** Resolve the region keys covering a lat/lng + radius viewport. */
export { getRegionsForBounds };

/**
 * Fetch every requested region concurrently. One dead upstream must not fail
 * the batch, so results are settled individually and rejections are dropped —
 * the same contract as the OSIRIS route this was ported from.
 */
export async function fetchCctvRegions(regions: string[]): Promise<{
  cameras: CctvCamera[];
  sources: Record<string, number>;
}> {
  const results = await Promise.allSettled(
    regions.map((r) => REGION_FETCHERS[r]?.() ?? Promise.resolve([])),
  );
